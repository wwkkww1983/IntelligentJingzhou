...
public class RSAUtils {
    ...
    public static Map<String, String> createKeys(int keySize){
        ...
        byte[] publicKeyStr = Base64.encode(publicKey.getEncoded(), Base64.DEFAULT);
//        String publicKeyStr = Base64.encodeToString(publicKey.getEncoded(), Base64.DEFAULT);
        //得到私钥
        Key privateKey = keyPair.getPrivate();
        byte[] privateKeyStr = Base64.encode(privateKey.getEncoded(), Base64.DEFAULT);
//        String privateKeyStr = Base64.encodeToString(privateKey.getEncoded(), Base64.DEFAULT);
        Map<String, String> keyPairMap = new HashMap<String, String>();
        keyPairMap.put("publicKey", new String(publicKeyStr));
        keyPairMap.put("privateKey", new String(privateKeyStr));
    ...
    public static String publicEncrypt(String data, RSAPublicKey publicKey){
        try{
            ...
            return new String(Base64.encode(rsaSplitCodec(cipher,
                    Cipher.ENCRYPT_MODE, data.getBytes(CHARSET),
                    publicKey.getModulus().bitLength()), Base64.DEFAULT));
        }catch(Exception e){
    ...
    public static String privateEncrypt(String data, RSAPrivateKey privateKey){
        try{
            ...
            byte[] resultData = rsaSplitCodec(cipher, Cipher.ENCRYPT_MODE,
                    data.getBytes(CHARSET), privateKey.getModulus().bitLength());
            return new String(Base64.encode(resultData, Base64.DEFAULT));
        }catch(Exception e){
            throw new RuntimeException("加密字符串[" + data + "]时遇到异常", e);
        }
    }
